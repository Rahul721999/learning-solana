...
pub struct Liquidate<'info> {
    #[account(mut)] // The liquidator who initiates the liquidation, must sign the transaction
    pub liquidator: Signer<'info>,

    pub price_update: Account<'info, PriceUpdateV2>, // Stores the latest price updates

    pub collateral_mint: InterfaceAccount<'info, Mint>, // The token used as collateral (e.g., SOL)
    pub borrowed_mint: InterfaceAccount<'info, Mint>,   // The token that was borrowed (e.g., USDC)
    ...
    pub collateral_bank: Account<'info, Bank>, // Bank holding collateral deposits
    ...
    pub collateral_bank_token_account: InterfaceAccount<'info, TokenAccount>, // Treasury account for collateral
    ...
    pub borrowed_bank: Account<'info, Bank>, // Bank holding borrowed assets
    ...
    pub borrowed_bank_token_account: InterfaceAccount<'info, TokenAccount>, // Treasury account for borrowed assets
    ...
    user_account: Account<'info, User>, // Stores liquidator's account details
    ...
    pub liquidator_collateral_token_account: InterfaceAccount<'info, TokenAccount>, // Liquidator's wallet to receive collateral
    ...
    pub liquidator_borrowed_token_account: InterfaceAccount<'info, TokenAccount>, // Liquidator's wallet to pay borrowed token for liquidation

    pub system_program: Program<'info, System>, // Required for account creation and SOL transfers
    pub token_program: Interface<'info, TokenInterface>, // Required for token transfers
    pub associated_token_program: Program<'info, AssociatedToken>, // Used for creating associated token accounts
}

/*
 * Liquidation process *
 *1. Check if user is under-collaterized
 *2. Calculate liquidation amount
 *3. Make a CPI transfer from the user's token account to the bank's token account
 *4. Update the user & bank states
...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()> {
    ...
    let health_factor =
        (total_collateral * collateral_bank.liquidation_threashold) / total_borrowed;
    ...
    // liquidator pays the borrowed amount back to the bank
    ...
    let transfer_to_bank = TransferChecked{
        ...
        mint: ctx.accounts.borrowed_bank_token_account.to_account_info(),
    ...
    let mint_key = ctx.accounts.borrowed_mint.key();
    let signer_seeds : &[&[&[u8]]] = &[
        &[
            b"treasury",
            mint_key.as_ref(),
