...
pub struct Borrow<'info> {
    #[account(mut)]
    pub signer: Signer<'info>, // the borrower
    pub mint: InterfaceAccount<'info, Mint>, // token borrwer wants to borrow
...
/*
 *1. Check if the user has enough collateral to borrow.
 *2. Warn if borrowing beyond the safe amount but still allow if within the man borrowable amount.
...
pub fn process_borrow(ctx: Context<Borrow>, amount: u64) -> Result<()> {
    ...
    // Check user hash enough collateral
    let price_update = &mut ctx.accounts.price_update;

    let total_collateral = match mint_key {
        // user wants to Borrow USDC
        key if key == user.usdc_address => {
            let sol_fee_id = get_feed_id_from_hex(SOL_USD_FEED_ID)?;
            let sol_price =
                price_update.get_price_no_older_than(&Clock::get()?, MAXIMUM_AGE, &sol_fee_id)?;
    ...
    let borrowable_amount = total_collateral as u64 * bank.liquidation_threashold;

    if borrowable_amount < amount {
        return Err(ErrorCode::OverBorrowableLimit.into());
    }
